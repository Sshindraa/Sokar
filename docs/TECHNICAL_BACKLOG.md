...
| ID      | Fichiers                                                                                              | Problème                                                               | Correction recommandée                                            | Test attendu                                   | Statut      |
| ------- | ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | ----------------------------------------------------------------- | ---------------------------------------------- | ----------- |
| SEC-004 | `apps/api/src/plugins/rate-limit.ts`                                                                  | Rate limit global 100 req/min sans distinction des endpoints sensibles | Rate limiting multi-niveaux (hold/confirm/erase plus stricts)     | Tests de rate limit par route                  | Non corrigé |
| SEC-005 | `apps/api/src/main.ts` + `test/test.routes.ts`                                                        | Routes de test protégées uniquement par `NODE_ENV`                     | Variable `ENABLE_TEST_ROUTES=false` explicite                     | Test que `/api/test` est 404 en prod           | Non corrigé |
| SEC-006 | `apps/api/src/main.ts` + `shared/observability/observability.routes.ts`                               | `/metrics` public sans auth                                            | Auth basique ou allowlist IP Prometheus                           | Test d'accès non autorisé                      | Non corrigé |
