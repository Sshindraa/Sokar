...
> **Auth globale** : Clerk (sauf routes explicitement publiques — MCP, voice webhook, public Canal A, RGPD `request-verification`, `confirm-link`, `privacy-policy`)
...
les futures routes `/public/r/:slug` de Canal A qui seront **anonymes**.
...
`canalAPublished`, `canalAAgentic`, etc.).
...
Module : `apps/api/src/modules/canal-a/` (à créer)
...
Voir [[Canal A P0]] et `docs/canal-a-v1.1.md` pour le détail.
...
| `/public/r/:slug/*` (Canal A) | 4 | **Public** | canal-a (T2) |
