...
- [ ] **Voice — naturalité phases 0 à 2** : PR A (métriques + fin de tour, flag coupé) puis ~20 appels scriptés de référence ; PR B (vitesse 1.2–1.4) ; PR C (phase 2 : annonce d'assistant virtuel, prompt, variété) après avis juridique AI Act art. 50. Le banc d'évaluation de la phase 4 doit inclure « vous êtes ouverts dimanche ? » posé à un restaurant **ouvert** le dimanche (risque de reprise d'un exemple du prompt).
