...
- [ ] **Observabilité R1-6** : Prometheus production scrape les quatre cibles ; Grafana est configuré en loopback, lecture seule via tunnel SSH, avec secret dédié hors du checkout. Les métriques TurnPlan attendent des appels réels. Triage distinct : 270 dead letters `evening-report` en staging (clé Resend absente) ; aucun rejeu ni suppression.
