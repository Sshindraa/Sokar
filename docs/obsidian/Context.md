...
- [ ] **Observabilité R1-6 / TurnPlan shadow** : Prometheus scrape les quatre cibles production/staging ; Grafana est actif en production, loopback-only et accessible en lecture via tunnel SSH. Le shadow TurnPlan est activé globalement sans autorité métier ; attendre des appels réels éligibles et lire l’accord **par dimension** avant d’activer `VOICE_TURN_PLAN_AUTHORITY_ENABLED` (canary additif, défaut off). Suivants : opération/provenance des faits, reprise scope-safe, proposal→confirm→commit pour annulation et carte cadeau, barrière de parole, cycle Telnyx du transfert.
