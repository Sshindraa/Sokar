...
- [ ] **Voice — fuseau des réservations téléphoniques** : corrigé pour les nouvelles réservations (PR #218, `4e28e85`). Vérifier `timedatectl` sur le VPS. Si le serveur n'était pas en Europe/Paris, recaler l'existant avec `pnpm --filter @sokar/api ops:phone-reservation-timezone -- --server-timezone <tz> --before <déploiement du correctif>` (aperçu), puis `--backup <fichier> --apply` après sauvegarde Postgres.
