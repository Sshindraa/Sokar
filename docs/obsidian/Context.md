...
- [ ] **Observabilité R1-6 / TurnPlan shadow** : Prometheus scrape les quatre cibles production/staging ; Grafana est actif en production, loopback-only et accessible en lecture via tunnel SSH. Le shadow TurnPlan est activé globalement sans autorité métier ; attendre des appels réels éligibles pour mesurer les accords.
- [ ] **Staging email / dead-letter** : Resend reste désactivé. Les 270 rapports obsolètes ont été écartés avec motif ; dead-letter vide et alerte résolue. `EVENING_REPORTS_ENABLED=false` est posé et les 14 schedulers retirés ; reste à déployer le garde-fou pour éviter leur recréation au redémarrage. Aucun rejeu ; prévoir une clé dédiée et une destination de test avant toute activation.
