...
| Multi-site                               | 249 € / mois + 99 € / établissement supplémentaire | Compte, OWNER, quota payé et deux sites isolés             | Deux sites, membre limité, changement de site, facture `siteCount`, dépassement de quota et rollback | Staging : troisième site créé, quatrième refusé hors quota, suspension/réactivation validées, sélecteur isolé, portail sandbox 447 €/mois et rejeu webhook signé validés ; membre Clerk réel et archivage du principal restent ouverts |
...
Chaque activation doit référencer le commit, les variables attendues, les migrations, les smoke tests, la sauvegarde et la commande de rollback. La projection Stripe multi-site et le modèle account/site restent additifs. La migration et le backfill sont validés sur staging (10/10 restaurants historiques), le rejeu webhook signé est validé (double réponse 200, signature invalide en 400), le troisième site est créé et le quatrième est refusé hors quota ; la release `main@724f6768` est déployée en staging et en production avec smoke tests verts. Les preuves d'isolation réelle, de droits membre, d'annuel/taxes/prorata/période de grâce et de cohorte pilote restent nécessaires avant l'activation commerciale autonome.
