...
- Un ajout est refusé avec `MULTI_SITE_SUBSCRIPTION_REQUIRED` lorsque le quota `RestaurantAccountBilling.entitledSiteCount` est atteint. Les webhooks Stripe projettent désormais le `siteCount` au niveau du compte ; il reste à valider sur staging avant l'ouverture commerciale du parcours.
...
- ne pas activer la facturation multi-site avant le test d'entitlement Stripe et le provisioning de deux sites.
