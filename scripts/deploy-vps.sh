...
if [ "${1:-}" = "rollback" ]; then
    ...
    if [ -z "$TARGET_RELEASE" ]; then
        ...
        log_section "Rollback Sokar — recherche de la release précédente"
        list_releases
        log info ""
...
if ! git diff --quiet || ! git diff --cached --quiet; then
    log_error "Fichiers suivis modifiés sur le VPS. Refus de les stasher automatiquement."
    git status --short
    exit 1
fi
...
recover_services() {
    ...
    trap - ERR
    log info ""
    log_error "Déploiement interrompu (code ${exit_code})."
    ...
    log info "   → Remise en ligne des services..."
    ...
    log info ""
    log_error "Services restaurés à l'état pré-build. Le déploiement a échoué."
...
log info "   Stopping dashboard + Sokar Connect (API stays up)..."
pm2 stop sokar-dashboard sokar-connect 2>/dev/null || true
...
if [ "${SOKAR_REEXECED:-0}" != "1" ] && [ "$PREV_HASH" != "$NEW_HASH" ]; then
    if git diff --name-only "$PREV_HASH" "$NEW_HASH" 2>/dev/null | grep -qE '^scripts/deploy-vps\.sh$'; then
        log info "   📎 deploy-vps.sh mis à jour par le pull — re-exec pour charger la nouvelle version..."
...
LAST_DEPLOYED_HASH=$(cat "$RELEASES_DIR/.latest-hash" 2>/dev/null || log info "")
...
if [ -z "$LAST_DEPLOYED_HASH" ]; then
    ...
    CHANGED_FILES=$(git diff --name-only "$LAST_DEPLOYED_HASH" "$NEW_HASH" 2>/dev/null || log info "")
    if [ -z "$CHANGED_FILES" ]; then
        ...
        log info "   📎 $local_count fichier(s) modifié(s) depuis $LAST_DEPLOYED_HASH"
...
log info ""
log info "📦 Installing Nginx routing..."
...
log info ""
log_section "Checking HTTP endpoints"
API_STATUS=$(curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:4000/health 2>/dev/null || echo "FAIL")
DASH_STATUS=$(curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:3000 2>/dev/null || echo "FAIL")
...
log info "   api (127.0.0.1:4000/health) → $API_STATUS"
log info "   dashboard (127.0.0.1:3000)  → $DASH_STATUS"
log info "   connect (127.0.0.1:4002/restaurant/chez-sokar-demo) → $CONNECT_STATUS"
...
if [ "$API_STATUS" = "200" ] \
    ...
    list_releases
    log info ""
    log info "   Pour rollback : bash scripts/deploy-vps.sh rollback"
