/**
 * Canal A — Page restaurant publique (T5 complète).
 *
 * URL : /r/[slug]
 *
 * Sections :
...
export async function generateMetadata({
  ...
  return {
    ...
    robots: {
      index: !isPreview,
      follow: true,
      googleBot: { index: !isPreview, follow: true, 'max-snippet': -1, 'max-image-preview': 'large' },
    },
  };
}
...
export default async function RestaurantPage({
  ...
  params: { slug: string };
  searchParams: { source?: string; utm_source?: string; utm_medium?: string; utm_campaign?: string; preview?: string };
}) {
  const isPreview = searchParams.preview === '1';
  const restaurant = await fetchPublicRestaurant(params.slug, { preview: isPreview });
  ...
  const cuisine = restaurant.cuisineTypes[0] ?? 'Restaurant';
  const mapsUrl = googleMapsUrl(restaurant.address.line1, restaurant.address.city, restaurant.address.country);
          ...
          {(restaurant.ambiance?.length ||
            ...
            <section className="mb-8 rounded-xl border border-border bg-background p-6">
              ...
              <ul className="space-y-2 text-ink">
                ...
                {restaurant.noiseLevel ? (
                  <li>
                    <span className="text-sm font-medium text-muted-foreground">Niveau sonore : </span>
                    {restaurant.noiseLevel}
                  </li>
                ) : null}
              </ul>
