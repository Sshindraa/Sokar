...
export type AnalyticsEvent =
  | { event: 'restaurant_page_view'; restaurantId: string; restaurantSlug: string; city: string; source?: string; utmSource?: string; utmMedium?: string; utmCampaign?: string }
  | { event: 'restaurant_book_cta_clicked'; restaurantId: string; restaurantSlug: string; source?: string }
  | { event: 'booking_page_view'; restaurantId: string; restaurantSlug: string; source?: string }
  | { event: 'availability_requested'; restaurantId: string; restaurantSlug: string; date: string; partySize: number }
  | { event: 'reservation_hold_created'; restaurantId: string; restaurantSlug: string; date: string; time: string; partySize: number; source?: string }
  | { event: 'reservation_confirmed'; restaurantId: string; restaurantSlug: string; reservationId: string; source?: string };
...
export function trackEvent(event: AnalyticsEvent): void {
  ...
  fetch(`${API_URL}/public/analytics/events`, {
    ...
    if (typeof console !== 'undefined') {
      console.error('[canal-a] analytics event failed', err);
    }
  });
}
