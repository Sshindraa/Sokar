 ...
 * directement @sokar/database. Cf. spec canal-a-v1.1 §6.6 :
 *   "apps/canal-a appelle GET /public/sitemap-data plutôt que
...
export async function fetchPublicRestaurant(
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[canal-a] fetch restaurant ${slug} failed: ${res.status}`);
      return null;
    }
    return (await res.json()) as PublicRestaurantDto;
  } catch (err) {
    console.error(`[canal-a] fetch restaurant ${slug} error:`, err);
    return null;
  }
}
...
export async function fetchAvailability(
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[canal-a] fetch availability ${slug} failed: ${res.status}`);
      return null;
    }
    return (await res.json()) as AvailabilityDto;
  } catch (err) {
    console.error(`[canal-a] fetch availability ${slug} error:`, err);
    return null;
  }
}
...
export async function fetchPublishedSlugs(): Promise<SitemapRestaurantEntry[]> {
  try {
    ...
    if (!res.ok) {
      console.error(`[canal-a] fetch sitemap-data failed: ${res.status}`);
      return [];
    }
    const data = (await res.json()) as { restaurants: SitemapRestaurantEntry[] };
    return data.restaurants;
  } catch (err) {
    console.error(`[canal-a] fetch sitemap-data error:`, err);
    return [];
  }
}
...
export async function fetchCities(): Promise<
  Array<{ city: string; citySlug: string; total: number; cuisines: Array<{ name: string; slug: string; count: number }> }>
> {
  try {
    ...
    const data = (await res.json()) as {
      cities: Array<{ city: string; citySlug: string; total: number; cuisines: Array<{ name: string; slug: string; count: number }> }>;
    };
    return data.cities;
  } catch (err) {
    console.error(`[canal-a] fetch cities error:`, err);
    return [];
  }
}
...
export async function fetchCityPage(
  ...
  try {
    ...
    console.error(`[canal-a] fetch city ${citySlug} error:`, err);
    return null;
  }
}
