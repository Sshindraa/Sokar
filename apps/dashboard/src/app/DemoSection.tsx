...
function RadialDial({ value }: { value: number }) {
  ...
  const innerStrokeDashoffset =
    innerCircumference - (Math.min(currentValue * 0.9, 100) / 100) * innerCircumference;
  return (
    ...
    <div className="space-y-2">
      ...
      <div className="relative flex h-8 items-center">
        ...
        <input
          ...
          className="relative z-10 w-full h-full appearance-none cursor-pointer accent-primary bg-transparent transition-all focus:outline-none focus:ring-0"
          style={{ minHeight: 44 }}
        />
      </div>
    </div>
...
function TelemetryTuner() {
  ...
  return (
    <div
      className="rounded-2xl border border-white/5 bg-white/[0.01] p-5 sm:p-6 flex flex-col justify-between shadow-xl relative overflow-hidden group transition-all duration-300 hover:border-white/10"
    ...
    >
      ...
      <div className="absolute top-3 right-4 text-[7px] font-bold text-white/10 font-mono tracking-widest pointer-events-none select-none hidden sm:block">
        SYS_OK
      </div>
    ...
    >
      ...
      <div className="relative z-10 mt-3 flex items-end justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs sm:text-[11px] font-bold text-white/40 tracking-wider uppercase font-sans">
            {label}
          </p>
        </div>
        {featured && (
    ...
    >
      ...
      <div className="relative z-10 mb-12 max-w-lg text-center">
        ...
        <p className="mt-2 text-xs md:text-sm text-white/50 leading-relaxed font-sans">
          Admirez l&apos;interface de pilotage Sokar en temps réel. Le moniteur d&apos;activité
          affiche les statistiques de l&apos;assistant vocal en parallèle de la console de dialogue.
        </p>
      </div>
