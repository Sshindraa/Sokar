...
const STEP_KEYS: OnboardingTaskKey[] = [
  'restaurant',
  'hours',
  'knowledge',
  'calendar',
    ...
    <main className="dark sokar-page relative min-h-screen overflow-hidden p-4 pt-28 md:p-8 md:pt-32">
      ...
      <div className="relative z-10 mx-auto max-w-6xl space-y-5">
        ...
        <section className="rounded-lg border border-border bg-card/90 p-4 shadow-xl backdrop-blur-xl transition-all duration-200 md:p-6">
          ...
          {step === 'canal-a-identity' && <CanalAIdentityStep />}
          {step === 'canal-a-location' && <CanalALocationStep />}
          {step === 'canal-a-cuisine' && <CanalACuisineStep />}
          {step === 'canal-a-capacity' && <CanalACapacityStep />}
          {step === 'canal-a-activation' && <CanalAActivationStep />}
        </section>
...
function PhoneStep() {
  ...
  async function handleTestCall() {
    ...
    try {
      ...
      window.setTimeout(() => router.push('/onboarding/canal-a-identity'), 1200);
    } catch (err: any) {
  ...
  async function handleSkip() {
    ...
    router.push('/onboarding/canal-a-identity');
  }
    ...
    <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-border bg-background/60 p-4 text-sm text-muted-foreground transition-all duration-200">
          Active le renvoi d’appel depuis l’opérateur du restaurant vers le numéro Sokar, puis
          lance le test.
        </div>
...
function CanalAIdentityStep() {
  ...
  function setDemoPhoto() {
    setCoverImageUrl('https://images.unsplash.com/photo-1550966871-3ed3cdb5ed0c?q=80&w=1000&auto=format&fit=crop');
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      router.push('/onboarding/canal-a-location');
    } finally {
      setSaving(false);
    }
  }
        ...
        <Field label="Adresse web (Slug)">
          <div className="relative flex items-center">
            ...
            <Input
              ...
              required
            />
            {checkingSlug && <Loader2 className="absolute right-3 animate-spin text-muted-foreground" size={16} />}
            {!checkingSlug && slugAvailable === true && <Check className="absolute right-3 text-emerald-400" size={16} />}
            {!checkingSlug && slugAvailable === false && <span className="absolute right-3 text-xs text-destructive">Indisponible</span>}
          </div>
          ...
          >
            {coverImageUrl ? (
              <div className="absolute inset-0 p-2">
                ...
                <img src={coverImageUrl} alt="Cover preview" className="w-full h-full object-cover rounded-lg" />
                <button
                  type="button"
...
function CanalALocationStep() {
  ...
  const [citySuggestions, setCitySuggestions] = useState<Array<{ nom: string; codesPostaux: string[] }>>([]);
  const [saving, setSaving] = useState(false);
  const [geocoding, setGeocoding] = useState(false);

  // Auto-complete city from API geo FR
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      try {
        const res = await fetch(`https://geo.api.gouv.fr/communes?nom=${encodeURIComponent(cityQuery)}&fields=nom,codesPostaux&limit=5`);
        if (res.ok) {
  ...
  // Auto-geocode with Nominatim when address is entered
  useEffect(() => {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      router.push('/onboarding/canal-a-cuisine');
    } finally {
      setSaving(false);
    }
  }
...
const CUISINES_PRESETS = ['Italien', 'Japonais', 'Français', 'Pizza', 'Burgers', 'Végétarien'];
const DIETARY_PRESETS = ['végétarien', 'vegan', 'sans gluten', 'halal', 'casher'];
const FEATURES_PRESETS = ['terrasse', 'groupe', 'privatisation', 'anniversaire', 'ouvert dimanche', 'brunch'];

function CanalACuisineStep() {
  const router = useRouter();
  const { patch, orgId } = useApi();
  const { state, updateTask } = useOnboarding();
  const restaurant = state!.restaurant;
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      router.push('/onboarding/canal-a-capacity');
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="space-y-5">
        <div>
          ...
          <div className="flex flex-wrap gap-1.5 mt-2">
            {cuisineType.filter((c) => !CUISINES_PRESETS.includes(c)).map((c) => (
              <span
                key={c}
                className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-muted text-muted-foreground"
              >
                {c}
                <button type="button" onClick={() => setCuisineType(cuisineType.filter((x) => x !== c))} className="hover:text-foreground">×</button>
              </span>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-foreground mb-2">Régimes alimentaires proposés</label>
        ...
        <div>
          <label className="block text-sm font-medium text-foreground mb-2">Atouts de l’établissement</label>
          <div className="flex flex-wrap gap-2">
...
function CanalACapacityStep() {
  ...
  const [cancellationPolicy, setCancellationPolicy] = useState<string>(specials.cancellationPolicy || 'Annulation gratuite jusqu’à 2 heures avant le service.');

  const [depositRequired, setDepositRequired] = useState<boolean>(specials.depositRequired || false);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      router.push('/onboarding/canal-a-activation');
    } finally {
      setSaving(false);
    }
  }
        ...
        <div className="rounded-lg border border-border bg-background/40 p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Demande une empreinte de carte à tes clients.</p>
            </div>
...
function CanalAActivationStep() {
  ...
  const [canalAPublished, setCanalAPublished] = useState<boolean>(exposure?.canalAPublished || false);
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
      ...
      <div className="space-y-4">
        <div className="rounded-lg border border-border bg-background/40 p-4 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">La page devient accessible et réservable en ligne.</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
        ...
        <div className="flex gap-2">
          {canalAPublished ? (
            <Button asChild>
              <a href={publicUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1">
        ...
        {celebrate && (
          <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm transition-all animate-fade-in">
            <div className="bg-card border border-border rounded-xl max-w-md w-full p-6 text-center shadow-2xl space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                La page de ton établissement est maintenant prête à recevoir ses premières réservations en ligne et à être découverte par les assistants IA.
              </p>
              <div className="bg-background/60 border border-border rounded-lg p-3 text-sm font-mono flex items-center justify-between select-all">
                ...
                <button type="button" onClick={handleCopy} className="text-muted-foreground hover:text-foreground shrink-0 transition-colors">
