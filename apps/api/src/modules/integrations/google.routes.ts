...
export async function googleRoutes(app: FastifyInstance) {
  ...
  // Initiate OAuth flow
  app.get('/integrations/google-calendar/auth', { preHandler: requireOrg() }, async (req, reply) => {
  ...
  app.get('/integrations/google-calendar/callback', async (req, reply) => {
    ...
    try {
      ...
      const publicUrl = process.env.PUBLIC_URL || 'http://localhost:4000';
      const frontendUrl = publicUrl.includes('localhost:4000') ? 'http://localhost:3000' : publicUrl;
      return reply.redirect(`${frontendUrl}/dashboard/settings?google_sync=error&message=${encodeURIComponent(err.message)}`);
    }
  });

  // Disconnect Google Calendar
  app.post('/integrations/google-calendar/disconnect', { preHandler: requireOrg() }, async (req, reply) => {
    const restaurantId = req.restaurantId!;
    ...
    try {
      ...
      return reply.status(500).send({ error: 'Failed to disconnect integration', message: err.message });
    }
  });
}
