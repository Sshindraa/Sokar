 ...
 * - source=chatgpt forcée à 'web' si canalAAgentic=false
...
describe('Canal A — Routes publiques', () => {
  beforeEach(async () => {
    vi.clearAllMocks();
  ...
  describe('GET /public/r/:slug', () => {
    it('retourne 200 + DTO si le restaurant est publié', async () => {
      ...
      expect(body.canalAAgentic).toBe(false);
    });
    ...
    it("retourne 404 si canalAPublished=false", async () => {
      vi.mocked(db.restaurant.findUnique).mockResolvedValue({
        id: RESTAURANT_ID,
        slug: SLUG,
        agenticOptIn: true,
        publishedAt: new Date('2026-06-24'),
        exposureSettings: {
          canalAPublished: false, // NOT published
          canalAAgentic: false,
        },
        images: [],
      } as any);
  ...
  describe('GET /public/r/:slug/availability', () => {
    it('retourne les slots du jour', async () => {
      ...
      vi.mocked(db.restaurantExposureSettings.findUnique).mockResolvedValue(mockExposureSettings as any);
  ...
  describe('POST /public/r/:slug/hold', () => {
    ...
    it("force source='web' si canalAAgentic=false et source=chatgpt", async () => {
      vi.mocked(db.restaurant.findUnique).mockResolvedValue({
        id: RESTAURANT_ID,
        slug: SLUG,
        agenticOptIn: true,
        ...
        exposureSettings: {
          canalAPublished: true,
          canalAAgentic: false, // PAS d'agentic
        },
        images: [],
      } as any);
      vi.mocked(db.restaurantExposureSettings.findUnique).mockResolvedValue(mockExposureSettings as any);
      ...
      expect(res.statusCode).toBe(200);
      const body = res.json();
      // chatgpt doit être forcé à 'web' car canalAAgentic=false
      expect(body.sourceNormalized).toBe('web');
    });

    it("préserve source='google' même si canalAAgentic=false (SEO)", async () => {
      vi.mocked(db.restaurant.findUnique).mockResolvedValue({
        id: RESTAURANT_ID,
        slug: SLUG,
        agenticOptIn: true,
