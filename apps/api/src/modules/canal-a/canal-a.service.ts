/**
 * Canal A — Service principal.
 *
 * Responsabilités :
 * - lookup restaurant par slug
 * - vérif gating (canalAPublished + canalAAgentic)
...
const CACHE_KEY_PREFIX = 'canal-a:restaurant:';

export class CanalAService {
  constructor(
   ...
   */
  async getPublishedBySlug(slug: string, options?: { preview?: boolean }): Promise<PublicRestaurantDto | null> {
    ...
    if (!isPreview) {
      try {
        ...
        logger.warn(
          ...
          'canal-a cache read failed, falling back to DB',
        );
      }
    }
    ...
    // Gating: la page publique requiert canalAPublished + publishedAt + slug
  ...
  async invalidateSlugCache(slug: string): Promise<void> {
    try {
      ...
      logger.warn(
        { err: err instanceof Error ? err.message : err, slug },
        'canal-a cache invalidate failed',
      );
    }
  }
  ...
  private toPublicDto(r: PublicRestaurantSource): PublicRestaurantDto {
    ...
    const gallery = r.images
      .filter((i) => !i.isCover && i.url !== cover)
      .map((i) => i.url);
